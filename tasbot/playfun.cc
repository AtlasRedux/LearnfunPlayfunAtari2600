...
static void SaveFuturesHTML(const vector<Future> &futures,
  ...
  for (int i = 0; i < futures.size(); i++) {
    ...
    for (int j = 0; j < futures[i].inputs.size(); j++) {
      out += SimpleInput::InputToColorString(futures[i].inputs[j]);
    }
    out += "</div>\n";
  }
  Util::WriteFile(filename, out);
...
struct PlayFun {
  PlayFun() : watermark(0), log(NULL), rc("playfun") {
    ...
    solution = SimpleInput::ReadInputs(moviename);
    ...
    if (resume_mode) {
      ...
      string progress_file = game + "-playfun-futures-progress.a26inp";
      vector<string> saved_subtitles;
      vector<uint8> saved = SimpleInput::ReadInputsAndSubtitles(
  ...
  // Reduced from NES values (40/800) because stella2023 retro_run()
  // is ~100x slower than FCEUX per frame (~0.7ms vs ~0.01ms).
  static const int NFUTURES = 20;

  // Number of futures that should be generated from weighted
  // motifs as opposed to totally random.
  static const int NWEIGHTEDFUTURES = 16;
  ...
  static const int MUTATEFUTURES = 4;

  // Number of inputs in each future.
  // Reduced for Atari's slower core. 300 frames = 5s of gameplay,
  // enough to detect dead ends like walls in underground passages.
  static const int MINFUTURELENGTH = 30;
  static const int MAXFUTURELENGTH = 300;
  ...
  void Master(const vector<int> &helpers) {
    ...
    for (;; iters++) {
      ...
      fprintf(stderr, "%d rounds, "
              ANSI_WHITE "%zu inputs" ANSI_RESET ". backtrack in %lld. "
              "Cxpoints at ",
              iters, movie.size(), (long long)rounds_until_backtrack);
  ...
  void MaybeBacktrack(int iters,
    ...
    if (*rounds_until_backtrack == 0) {
      *rounds_until_backtrack = TRY_BACKTRACK_EVERY;
      ...
      SimpleInput::WriteInputsWithSubtitles(
          StringPrintf("%s-playfun-backtrack-%d-replaced.a26inp",
                       game.c_str(), iters),
          game + ".a26",
          BASE64,
          movie,
          SimpleInput::MakeSparseSubtitles(subtitles));
      ...
      SimpleInput::WriteInputsWithSubtitles(
          StringPrintf("%s-playfun-backtrack-%d-replacement.a26inp",
                       game.c_str(), iters),
          game + ".a26",
          BASE64,
          movie,
  ...
  void SaveMovie() {
    ...
    SimpleInput::WriteInputsWithSubtitles(
        game + "-playfun-futures-progress.a26inp",
        game + ".a26",
        BASE64,
        movie,
        SimpleInput::MakeSparseSubtitles(subtitles));
  ...
  void SaveDiagnostics(const vector<Future> &futures) {
    ...
    for (int i = 0; i < futures.size(); i++) {
      ...
      for (int j = 0; j < futures[i].inputs.size(); j++) {
        ...
        SimpleInput::WriteInputs(StringPrintf("%s-playfun-future-%d.a26inp",
                                            game.c_str(),
                                            i),
                               game + ".a26",
                               BASE64,
...
int main(int argc, char *argv[]) {
  ...
  if (!resume_mode) {
    ...
    string g = cfg["game"];
    if (!g.empty()) {
      string progress_file = g + "-playfun-futures-progress.a26inp";
